...
#détermination du profil de consommation, ici Consommation en "stetson plat" 9h-18h avec JO (lun-ven) et JC (sam-dim)
Hstart=9
Hend=18
selector_HP=((hour>=Hstart)  & (hour<=Hend)& (weekday<5 ))
...
####Calcul des puissances P0 et P1  pour le profil de consommation   
    P0= (df.EHCH[i]+df.EHCB[i])/(hch[i]+hcb[i])  # calcul P0  puissance talon en heure creuse 
    Pui_HC.append(P0) # création de la liste puissance en heure creuse (puissance talon P0)
    P1=(df.EHPH[i]+df.EHPB[i]-Pui_HC[i]*(df.Nbr_jours_chomés[i]*16+df.Nbr_jours_ouvrés[i]*(16-(Hend-Hstart))))/(df.Nbr_jours_ouvrés[i]*(Hend-Hstart))  
    Pui_HP.append(P1)
